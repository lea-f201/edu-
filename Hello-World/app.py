...
with col[2]:
    ...
    df["Governorate"] = df["refArea"].str.extract(r".*/(.*)_Governorate")[0].str.replace("_", " ").str.strip()

    # Coordinate map
    gover_coords = {
        "Baalbek El Hermel": (34.545895, 36.16667),
        "Akkar": (34.208272, 36.2625889),
        "North": (34.4362, 35.8497),
    ...
    # Match names and assign coordinates
    gov_df = df[df["Governorate"].notna()]
    avg_df = gov_df.groupby("Governorate")[[selected_edu]].mean().reset_index()
    avg_df["Latitude"] = avg_df["Governorate"].map(lambda x: gover_coords.get(x, (None, None))[0])
    avg_df["Longitude"] = avg_df["Governorate"].map(lambda x: gover_coords.get(x, (None, None))[1])

    # Drop rows with invalid/missing coordinates
    avg_df = avg_df.dropna(subset=["Latitude", "Longitude"])
