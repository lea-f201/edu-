...
with st.sidebar:
    ...
    if 'quotes' not in st.session_state:
        ...
        "Education levels across all categories are consistently lower in the Nabatieh governorate compared to the national averages. This suggests a strong opportunity for NGOs and policymakers to prioritize educational support in that region.",
        "More than 10 towns in Lebanon have a notably low percentage of residents who have completed university education. These towns include, but are not limited to, Ebra W Chouan, Tallousa, and Aaychiyeh.",
        "The highest rates of higher education attainment are concentrated in towns located along Lebanon’s western coastline.",
        "Across Lebanon, higher education represents the least attained level of education. This may indicate that residents who reach this stage often continue directly to university studies, bypassing prolonged stops at the 'higher education' level."
        ]

    def display_quote():
       quote = st.session_state.quotes[st.session_state.count]
       st.markdown(f"<p style='text-align:left'>{quote}</p>", unsafe_allow_html=True)
...
with col[0]:
    st.subheader("Average Across Lebanon of Last Education Level Secured for")
    st.markdown(f"**{selected_edu}**")
    st.markdown(f"**{round(df[selected_edu].mean()*100,2)}%**", unsafe_allow_html=True)
    st.markdown("---")
    st.subheader("Proportion of Population that Completed")
    st.markdown(f"**{selected_edu}**")
    st.markdown(f"{round(get_cum(selected_edu)*100,2)}%")
...
with col[3]:
    ...
    st.dataframe(df_selected_town_edu,
                 ...
                 column_config={
                    ...
                    selected_edu: st.column_config.ProgressColumn("Highest Level Achieved", format="%f", min_value=0, max_value=1)
                 })
    with st.expander('About', expanded=True):
        st.markdown("""
            - Data from https://linked.aub.edu.lb:8502/  
            - Layout and base code from [Streamlit Blog](https://blog.streamlit.io/crafting-a-dashboard-app-in-python-using-streamlit/)  
            - Gemini and CharlyWargnier GitHub helped with UI and buttons
        """)

df["refArea"] = df["refArea"].str.split("/", n = 4).str[4] 
df["governorate"] = df["refArea"].str[-1] == "e"
gover_df = df[df["governorate"]]
